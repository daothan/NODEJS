...
exports.store = (event, context, callback) => {
  const requestBody = JSON.parse(event.body);
  const fullname = requestBody.fullname;
  const email = requestBody.email;
  const experience = requestBody.experience;
  console.log(fullname, email, experience);
...
exports.show = (event, context, callback) => {
  ...
  .then(result => {
    ...
    callback(new Error('Whoops! Something went wrong'));
    return;
  })
}
exports.deleteCandidate = (event, context, callback) => {
  const params = {
    TableName: process.env.CANDIDATE_TABLE,
    Key: {
      id: event.pathParameters.id
