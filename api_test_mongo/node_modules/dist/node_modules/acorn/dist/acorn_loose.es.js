...
exports.pluginsLoose = exports.LooseParser = exports.parse_dammit = undefined;
...
var LooseParser = function LooseParser(input, options) {
  ...
  this.tok = this.last = { type: _acorn.tokTypes.eof, start: 0, end: 0 };
...
LooseParser.prototype.dummyNode = function dummyNode(type) {
  ...
  this.last = { type: _acorn.tokTypes.name, start: dummy.start, end: dummy.start, loc: dummy.loc };
  return dummy;
};
  ...
  for (;;) {
    try {
      ...
      if (/unterminated/i.test(msg)) {
        ...
        if (/string/.test(msg)) {
          replace = { start: e.pos, end: pos, type: _acorn.tokTypes.string, value: this$1.input.slice(e.pos + 1, pos) };
        } else if (/regular expr/i.test(msg)) {
          var re = this$1.input.slice(e.pos, pos);
          try {
            re = new RegExp(re);
          } catch (e) {/* ignore compilation error due to new syntax */}
          replace = { start: e.pos, end: pos, type: _acorn.tokTypes.regexp, value: re };
      ...
      this$1.resetTo(pos);
      if (replace === true) {
        replace = { start: pos, end: pos, type: _acorn.tokTypes.name, value: "✖" };
      }
      if (replace) {
...
lp$1.parseStatement = function () {
  ...
  switch (starttype) {
    case _acorn.tokTypes._break:case _acorn.tokTypes._continue:
      this.next();
  ...
  while (!this.closes(_acorn.tokTypes.braceR, indent, line)) {
    ...
    if (isDummy(method.key)) {
      if (isDummy(this$1.parseMaybeAssign())) {
        this$1.next();
      }this$1.eat(_acorn.tokTypes.comma);continue;
    }
...
lp$2.parseExprAtom = function () {
  ...
  switch (this.tok.type) {
    ...
    case _acorn.tokTypes.num:case _acorn.tokTypes.string:
      node = this.startNode();
    ...
    case _acorn.tokTypes._null:case _acorn.tokTypes._true:case _acorn.tokTypes._false:
      node = this.startNode();
